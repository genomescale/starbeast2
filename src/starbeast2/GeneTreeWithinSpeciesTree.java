...
public class GeneTreeWithinSpeciesTree extends CalculationNode {
    public Input<TreeInterface> treeInput = new Input<>("tree", "Tree object for this gene.");
    ...
    protected void addSpeciesOccupancy(Map<Node, double[]> occupancyMap) {
        final Node[] geneTreeNodes = treeInput.get().getNodesAsArray();

        for (int i = 0; i < geneTreeNodeCount; i++) {
            occupancyMap.put(geneTreeNodes[i], speciesOccupancy[i]);
        }
    }
}
