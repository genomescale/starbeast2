...
relatively short, and relatively distantly spaced along the genome. Typical
sources of good that fits the StarBEAST2 model include RAD-seq loci
\citep{Ogilvie2016} or exons \citep{Scornavacca2017}.
...
model rates and base frequencies for those gene trees, per-species-lineage population
sizes and per-species-lineage molecular clock rates.
...
and the use of fossil data for tip-dated analyses. Fossil data can include ancient
...
This tutorial will cover using StarBEAST2 (i) to estimate a species tree and
per-species-lineage population sizes of the genus \textit{Canis} from exon sequences,
(ii) to estimate per-species-lineage relative molecular clock rates, and (iii) to
conduct a total evidence tip-dating analysis where species divergence times
are estimated in millions of years.

\section{Preliminaries}

Before doing anything, we need to install StarBEAST2, which is a package for
...
After downloading and installing BEAST2 on your computer, open BEAUTi, which
is the graphical user interface for BEAST2. To install new packages (including
StarBEAST2), open the File menu and select Manage Packages (Figure~\ref{fig:managePackages}).
...
Scroll down (if necessary) to StarBEAST2 and click the install button, which will take
care of the installation for you (Figure~\ref{fig:installStarBEAST2}). After the installation of StarBEAST2 is finished, just like \textbf{for any BEAST2 package}, you must
quit and relaunch BEAUTi.
...
After relaunching BEAUTi, you will notice four new templates. The first
...
enable per-species-lineage clock rates, as described by \cite{Ogilvie2016}.
The relaxed clock models used by those templates are uncorrelated log-normal (UCLN),
relaxed clock clock (RLC), and uncorrelated exponential distribution (UCED)
respectively.
...
FigTree to view summary trees, and Tracer to check some parameters and
...
``CanisPhylogeny''. (Re)launch BEAUTi and load up the strict clock StarBEAST2
template by selecting StarBEAST2 from the Templates submenu in the File menu
...
which is for later on in the tutorial.
...
All 16 files are MSAs of nucleotide sequences, so select ``all are nucleotide''
...
For almost all StarBEAST2 analyses you should link the clock models. Rates between
loci are allowed to vary and the mean rate across all sites is fixed at 1. Therefore
by linking the clocks, the unified clock rate can be fixed at an \textit{a priori} value
to calibrate the analysis, or left estimated when node-dating or tip-dating is used.
...
Note that linking site models in StarBEAST2 v14 will unify the substitution rates of
...
For this tutorial, each species has two haplotypes ``a'' and ``b'', both from
...
click the Guess button. To assign the right names, keep ``use everything''
selected, but change ``after first'' to ``before last''. Leave the underscore
in the text box (Figure~\ref{fig:guessTaxonMap}. This way everything before
...
Based on how we assigned the species name, there should be two haplotypes
...
Ignore the Tip Dates tab, which is for tip-dating and will be covered later in
the tutorial. Open the Gene Ploidy tab, and observed that the default value
...
by 2.0. If you use any mitochondrial or Y or W chromosomal loci, you should
change their ploidy to 0.5, because there is on average only 0.5 copies per
individual. The ploidy of X or Z chromosomal loci should be set to 1.5 for the
same reason.
...
categories will be be $4\times$ slower. That likelihood is a major
...
$1^{-3}$ substitutions per site per million years. While we don't know
the average rate across all loci in our data within \textit{Canis}, we can
use it as a \textbf{very rough} calibration. Open the clock model panel
...
/path/to/beast to match the folder where BEAST is installed on your system.
...
``species.trees'', and separate file will be created for each gene tree. The command line
...
ScaleOperator(bdTreeRootScaler.t:Species)                               0.9259       9555     109831     0.0019     0.0800 Try setting scaleFactor to about 0.962
...
we will use Tracer. Open the Tracer app and then open the ``starbeast.log'' file.
...
and the speciescoalescent (which is the sum of log coalescent probability for
...
Tracer computes effective sample sizes (ESS) for each loggest statistic and
parameter. As a rule, ESS values should be at least 200, particularly for
important statistics and for parameters of interest. Because of the stochastic
nature of MCMC, your values \textbf{will be different} to those in
Figure~\ref{fig:tracerPosterior}.
...
clade posterior probabilities, select ``View clade toolbar'' of the Window menu.
...
You can see that using a fixed clock rate of $1^{-3}$ substitutions per site
...
to rely entirely on a summary tree. This is because for most clades and data
...
Set the burnin percentage to 10, select the ``species.trees'' file generated by
...
actually scaled by generation time. That means if generation times (the average
...
population sizes in number of individuals, divide each value by the
generation time. So if the scaled effective population size is 1, that
will correspond to 200,000 individuals.

\section{Estimating per-species-lineage clock rates}
